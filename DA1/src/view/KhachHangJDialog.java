...
public class KhachHangJDialog extends javax.swing.JDialog {
    ...
    private void tblKhachHangMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblKhachHangMouseClicked
        // TODO add your handling code here:
         if (evt.getClickCount() == 1) { // Hoặc 2 nếu cần double-click
        int row = tblKhachHang.getSelectedRow();
        if (row != -1) { 
            Integer id = Integer.valueOf(tblKhachHang.getValueAt(row, 0).toString()); // Cột ID
            String ten = tblKhachHang.getValueAt(row, 2).toString(); // Cột Tên
            String sdt = tblKhachHang.getValueAt(row, 5).toString(); // Cột SĐT

          

            // Gán dữ liệu vào biến selectedCustomer
            selectedCustomer = new KhachHang(id, ten, sdt);
